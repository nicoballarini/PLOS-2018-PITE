...
n_biom   =   1000
# Set to onlyPlots = TRUE if simulations already run, and only plots to be calculated
onlyPlots = FALSE

# Set the environment to store results by creating relative paths --------------
rproj <- rprojroot::find_package_root_file()
path  <- paste0(rproj,sprintf("/sim/normal_tol.beta0/nbiom%s/",n_biom))
...
effects <- expand.grid(b= c(0,1),   d1=c(0,1), d2=c(0,1/2))[c(1,5,3,7,2,8),]
...
if (!onlyPlots){
  for (j in c(2, 4, 3, 1)) {
    for (i in c(4,1,2,3,5,6)) {
      npergroup = npergroup.[j]
      case = i
      listdata = simulateNtrial_many(nsim = nsim,
                                     effects = all.eff,
                                     case = i,
                                     npergroup = npergroup,
                                     mc.cores = mc.cores,
      ...
      listdata5 <- do.call(rbind, lapply(listdata$dataset, function(x) (x[[5]])))
      ...
      saveRDS(listdata5, file = paste0(filename,"-seed.rds"))
      rm(listdata)
      rm(listdata1)
      rm(listdata2)
      rm(listdata3)
...
unique(results.coeficients$term)
