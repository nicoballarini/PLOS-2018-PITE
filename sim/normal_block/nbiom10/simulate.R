...
path  <- paste0(rproj,sprintf("/sim/normal_block/nbiom%s/",n_biom))
...
if (!onlyPlots){
  for (j in c(4, 3, 2, 1)) {
    for (i in 1:6) {
      ...
      case = i
      listdata = simulateNtrial_block(nsim = nsim,
                                effects = all.eff,
                                case = i,
                                npergroup = npergroup,
                                mc.cores = mc.cores,
...
for(i in files.2.coef){
  ...
  head(listdata3)
  listdata3 %>% filter(term %in% c("treatment","mkr1","mkr2","mkr6",
                                   "treatment:mkr6",
                                   "treatment:mkr2",
                                   "treatment:mkr1",
                                   "mkr1:treatment",
                                   "mkr2:treatment",
                                   "mkr6:treatment")) -> listdata3
  results.coeficients <- rbind(results.coeficients,
                               data.frame(listdata3, scenario=i))
}


# head(results.coeficients)
...
results.coeficients %>%
  mutate(term = ifelse(term=="mkr1:treatment", "treatment:mkr1",
                       ifelse(term=="mkr2:treatment", "treatment:mkr2",
                              ifelse(term=="mkr6:treatment", "treatment:mkr6", term)))) -> results.coeficients
...
tmp.dat <- data.frame(variable=c("a","b","g1","0","g2","d1","0","d2"),
                      term=c("(Intercept)", "treatment",
                             "mkr1", "mkr2", "mkr6",
                             "treatment:mkr1", "treatment:mkr2", "treatment:mkr6"))
reshape2::melt(effects, "case") %>% #head()
  left_join(tmp.dat) -> tmp.dat2


# Coverage
results.coeficients %>% #head()
  left_join(tmp.dat2[,-2]) %>% #head()
